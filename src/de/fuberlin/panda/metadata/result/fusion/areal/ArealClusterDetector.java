 ...
 * {@link ClusterDetector#detectClusters(Set)} method for eliminating all areal
 * outliers without a areal information before detecting the clusters. 
 * It also overrides the {@link #calculateNeighborhood(ClusteringObject, Set)}
 * method.
 * 
 * @author Sebastian Schulz
 * @since 24.02.2014
 */
public class ArealClusterDetector extends ClusterDetector {
	private static Logger logger = Logger.getLogger(MetadataConnector.class.getName());
	
	public ArealClusterDetector(double neighborhoodRadius, int minNeighbors) {
		super(neighborhoodRadius, minNeighbors);
	}

	public ArealClusterDetector(double neighborhoodRadius, int minNeighbors, double maxOutlierFactor) {
		super(neighborhoodRadius, minNeighbors, maxOutlierFactor);
	}
	
	/**
	 * This method calls {@link #eliminateOutliersWithoutGeocoords(Set)} to remove 
	 * {@code ArealClusteringObject}s without a timestamp and calls the 
	...
	public ClusteringResult detectClusters(TreeSet<? extends ClusteringObject> clusteringObjects) {
		eliminateOutliersWithoutGeocoords(clusteringObjects);
		return super.detectClusters(clusteringObjects);
	}
	
	/**
	 * This method removes all {@code ArealClusteringObject}s which doesn't have geographical
	 * coordinates (marked by latitude and longitude = null) from the {@code Set} of
	 ...
	 * 	analyzed.
	 */
	protected void eliminateOutliersWithoutGeocoords(TreeSet<? extends ClusteringObject> clusteringObjects) {
		Iterator<? extends ClusteringObject> iterator = clusteringObjects.iterator();
		while (iterator.hasNext()) {
			ArealClusteringObject clusteringObject = (ArealClusteringObject) iterator.next();
			if ((clusteringObject.getLatitude() == null)
				&& (clusteringObject.getLongitude() == null)) {
				clusteringObject.setNoMetadata();
				augmentedClusteringObjectList.add(clusteringObject);
				iterator.remove();
				logger.debug("Removed clustering object without areal metadata information");
			}
		}
	}
	 ...
	 * of a {@code ArealClusteringObject}.<br><br>
	...
	protected void calculateNeighborhood(ClusteringObject clusteringObject,
		...
		for (ClusteringObject neighbor : clusteringObjects) {
			if (!neighbor.equals(clusteringObject)) {
				ArealClusteringObject arealNeighbor = (ArealClusteringObject) neighbor;
				double distance = clusteringObject.getDistance(arealNeighbor);
